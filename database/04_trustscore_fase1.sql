SEMANA 4: TrustScore - Fase 1 (Validación GPS)

-- 1. Función validadora (GPS y Coherencia Anti-Spam)
CREATE OR REPLACE FUNCTION validar_gps_reporte()
RETURNS TRIGGER AS $$
DECLARE
    reportes_previos INT;
BEGIN
    -- 1. Validación de errores de hardware de GPS (0,0)
    IF ST_X(NEW.geom) = 0 AND ST_Y(NEW.geom) = 0 THEN
        RAISE EXCEPTION 'Rechazo automatico: Coordenadas de GPS invalidas (0,0)';
    END IF;

    -- 2. Validación de límites reales del mapa
    IF ST_Y(NEW.geom) < -90 OR ST_Y(NEW.geom) > 90 OR ST_X(NEW.geom) < -180 OR ST_X(NEW.geom) > 180 THEN
    ...
    -- Cuenta reportes del mismo usuario, misma categoría, a menos de 50 metros, en los últimos 10 minutos
    SELECT COUNT(id)
    ...
    IF reportes_previos > 0 THEN
        RAISE EXCEPTION 'Rechazo automatico: Incoherencia geoespacial. Ya enviaste este mismo reporte en esta area hace un momento.';
    END IF;
